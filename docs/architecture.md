...
The orchestrator in `main.rs::orchestrator` (`main.rs:213`) selects on three
...
### 5. State update + sample recording (`main.rs:401`)
...
- For each window, `record_sample(&mut histories[window.id], &w)` appends a
  ...
  (480 samples ≈ 16h at 120s baseline; `main.rs:60`).

### 6. Burn projection (`burn::compute_burn`, `main.rs:440`)
...
- **Pct-only suppression** (`main.rs:compute_burn` and
  ...
  only when `rate > 0` — see `decide_burn_row` (`burn.rs:228`).
...
documents this; the bucket selector is in `main.rs:465`).
...
### 9. Threshold notification (`main.rs:511`)
...
fail_streak)` (`scheduler.rs:24`) returns the next sleep in ms:
...
One `#[tokio::main]` runtime (`main.rs:112`), `rt-multi-thread`. Tasks:
...
The bucket rank for the chip is computed in `main.rs:465`; the threshold
notifications gate on `prev_rank < new_rank` at `main.rs:511`.
...
- **Two launches with the same `--instance=`** — second instance's
  `Lock::acquire` returns `Ok(None)` (`lock.rs:64`), main loop prints
  `"another instance is already running; exiting"` and returns 0
  (`main.rs:138`).
- **Stale PID lock after a crash** — `Lock::acquire` checks
  `/proc/<pid>` and takes over if the holder is dead (`lock.rs:90`).
...
| Menu doesn't render                            | dbusmenu `MENU_PATH` set to `/Menu` (`sni.rs:49`), `apply_menu` (`sni.rs`) |
| Burn row shows when it shouldn't               | `burn::decide_burn_row` (`burn.rs:228`), `cfg.burn_warning.use_epoch_average` |
| Wrong endpoint hit                              | `Config::endpoint`, `Config::shape` (`config.rs:26`) |
| Two trays colliding                            | `instance::name()` (`instance.rs:70`), `Lock::acquire` (`lock.rs:30`) |
| Key not sticking                                | `keyring.rs:13-26` — the secret-tool subprocess rationale |
| Polling too fast / too slow                    | `scheduler::next_interval` (`scheduler.rs:24`), `cfg.thresholds` |
| "another instance is already running" but it's not | `Lock::acquire` takeover logic (`lock.rs:64`) — manually `rm` `$XDG_RUNTIME_DIR/<inst>.pid` |
